...
$(document).ready(function()
{
    ...
    {   
        ...
        if( $.trim($('#search-location').val()) == '' )
        {
            ...
            document.getElementById("search-location").placeholder = 'Where do you want to go?';
            resetSearchForm();
            recentSearchOnFocus();
        }
...
function selectRecentSearchData(obj)
{
    ...
    else
    {
        ...
        //$('#cselect-search').find('div[data-value="'+guest+'"]').trigger('click');   

        setTimeout(function()
        {
            $('body #cselect-search .cselect-option[data-value="'+guest+'"]')[0].click();
        },2);    

        setTimeout(function()
        {
            $("#search-location").removeAttr('placeholder');
        },2);


        $("#search-guest").val(guest);
        $("#searchlng").val(lat);
        $("#searchlat").val(lng);
        ...
        if(dpd1 !="" && dpd2 != "")
        {
            ...
            $("#checkin_search").val(dpd1);
            $("#checkout_search").val(dpd2);
        ...
        if( $.trim($("#search-dp1").val()) == '' || $.trim($("#search-dp2").val()) == '' )
        {
           showSearchCheckIn(); 
           //showCheckin()
           //showMobileCheckIn();
...
function storeToLocalStorage()
{
    ...
    {   
        ...
        if( $.trim($("#checkin-hidden-search").val()) == '' || $.trim($("#checkout-hidden-search").val()) == '' )
        {   
   ...
   var address = $("#search-location").val();
   var dpd1 = $("#checkin_search").val();
   var dpd2 = $("#checkout_search").val(); 
   var guest = $("#guest").val();
   $("#search-guest").val(guest);
   //alert("dpd1 "+ dpd1 + " dpd2  "+ dpd2+ "guest "+ guest);


   var search_country = $("#adrs-country").val();
   var search_state = $("#adrs_state").val();
   var search_city = $("#adrs_city").val();
   var search_area = $("#adrs_area").val();
    ...
    for(var item in data)
    {
        ...
        if( replace_special_char(data[item]['address']) == replace_special_char(obj['address'])  )
        {
            ...
            /* check for price if price updated for the same search */
                ...
                ( $.trim(obj.minvalue) != '' &&  $.trim(obj.maxvalue) != '') 
